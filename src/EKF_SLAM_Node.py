...
class EKFSLAMNode(object):
    ...
    def __init__(self):
        ...
        self.R_motion = np.matmul(self.F.T, np.matmul(np.diag([0.00005, 0.00005, 0.00002]), self.F))

        # measurement noise (single observation)
        self.Q_meas = np.diag([0.0025, 0.0025])
    ...
    def update_pitch(self, msg):
        self.KalmanAngle = radians(msg.data)
    ...
    def predict(self, odom):
        ...
        dist_L = odom.dist_R - self.dist_off_L
        dist_R = odom.dist_L - self.dist_off_R

        # integrate system dynamics
        self.dist = (dist_L + dist_R)/2
        delta = self.dist - self.dist_last
        yaw_odom = (dist_L - dist_R) / 0.18  # Todo: check wheelbase
        x_p = self.x[0:3] + np.array([cos(self.x[2]) * delta, sin(self.x[2]) * delta, yaw_odom - self.x[2]])
    ...
    def correct(self, msg):
        ...
        for i, tag in enumerate(msg.detections):
            ...
            if int(tag.id[0]) not in self.landmark_ID:
                ...
                self.x[(3 + 2 * j):(3 + 2 * j + 2)] = self.x[0:2] + rot_mat.dot(landmark_pos)
            ...
            z_hat = rot_mat.T.dot(self.x[(3 + 2 * j):(3 + 2 * j + 2)] - self.x[0:2])


            # get H matrix for a single landmark
            H = np.zeros((2, 3 + 2 * self.n_landmarks))
            H[0, 0] = -cos(self.x[2])
            H[0, 1] = -sin(self.x[2])
            H[1, 0] = sin(self.x[2])
            H[1, 1] = -cos(self.x[2])
            H[0, 3 + 2 * j] = cos(self.x[2])
            H[0, 3 + 2 * j + 1] = sin(self.x[2])
            H[1, 3 + 2 * j] = -sin(self.x[2])
            H[1, 3 + 2 * j + 1] = cos(self.x[2])
        ...
        if self.visualize and self.timer == 10:
